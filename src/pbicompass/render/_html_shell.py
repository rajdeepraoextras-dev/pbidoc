...
/* These controls float over the document, so they are sized and coloured as a
   real toolbar rather than inheriting body's 16px reading size. They previously
   styled themselves with --border/--surface/--text/--accent, none of which this
   shell defines: an undefined var makes the whole declaration invalid, so the
   border was dropped outright and the background fell back to transparent,
   leaving two unstyled labels sitting on a stray shadow. Every token below is
   one :root actually declares — dark mode then follows for free. */
...
/* Save is the action that produces the deliverable, so it carries the fill;
   Edit stays quiet until it is actually toggled on. */
.document-editor #save-document {
  background: var(--primary);
  border-color: var(--primary);
  color: #fff;
}
.document-editor #save-document:hover {
  background: var(--primary-hover);
  border-color: var(--primary-hover);
  color: #fff;
}
/* "Editing is on" — a tint rather than Save's solid fill, so the two buttons
   stay tellable apart while the document is editable. */
.document-editor button[aria-pressed="true"] {
  background: var(--primary-light);
  border-color: var(--primary);
  color: var(--primary);
}
/* --primary-light is a near-white tint that :root never redarkens, so the
   pressed state needs its own dark value or it glares. Both selectors are
   required: the media query for system preference, the attribute for the
   sidebar toggle overriding it. */
@media (prefers-color-scheme: dark) {
  :root:not([data-theme="light"]) .document-editor button[aria-pressed="true"] {
    background: rgba(18, 79, 237, 0.24);
    color: #c7d7ff;
  }
  :root:not([data-theme="light"]) .document-editor button {
    box-shadow: 0 1px 2px rgba(0, 0, 0, 0.30), 0 4px 12px rgba(0, 0, 0, 0.35);
  }
}
:root[data-theme="dark"] .document-editor button[aria-pressed="true"] {
  background: rgba(18, 79, 237, 0.24);
  color: #c7d7ff;
}
:root[data-theme="dark"] .document-editor button {
  box-shadow: 0 1px 2px rgba(0, 0, 0, 0.30), 0 4px 12px rgba(0, 0, 0, 0.35);
}
...
/* --secondary, not the undefined --accent this used to name. */
#document-content[contenteditable="true"]:focus { outline-color: var(--secondary); }
