...
| `--model` | Model id (default `claude-opus-4-8`). For MeshAPI use `provider/model-name`, e.g. `inclusionai/ling-2.6-flash` (the MeshAPI default; also settable via the `MESHAPI_MODEL` env var). |
